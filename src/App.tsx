...
// IU Web / IndependentlyUNITED — Sample Landing Page (React + Tailwind)
// Notes:
// - Default export a single React component for preview.
// - Clean, minimal aesthetic; grid-based layout; rounded-2xl cards.
...
export default function IUWebLanding() {
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-slate-50 via-white to-slate-50 text-slate-900">
      ...
      {/* HERO */}
      <Section className="pt-16 md:pt-28" id="hero">
            ...
            <Card className="p-0 overflow-hidden">
              <div className="relative aspect-[4/3] w-full bg-[radial-gradient(ellipse_at_center,rgba(15,23,42,0.9),rgba(2,6,23,1))] text-white grid place-items-center">
                ...
                <div className="absolute inset-0 pointer-events-none">
                  {[...Array(14)].map((_, i) => (
                    <div key={i} className="absolute w-1 h-1 bg-white/80 rounded-full" style={{
                      top: `${10 + (i * 6) % 80}%`,
                      left: `${5 + (i * 13) % 90}%`,
                      opacity: 0.6
                    }} />
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            ...
            <div>
              ...
              <div className="relative aspect-[4/3] rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 overflow-hidden">
                {[...Array(18)].map((_, i) => (
                  <div key={i} className="absolute w-2 h-2 bg-slate-900 rounded-full" style={{
                    top: `${8 + (i * 11) % 85}%`,
                    left: `${6 + (i * 17) % 88}%`,
                    opacity: 0.9
                  }} />
                ))}
                {/* connecting lines using pseudo elements via inline SVG */}
